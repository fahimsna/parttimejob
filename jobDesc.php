...
foreach ($lines as $line) {
    ...
    while (true) {
        try {
            ...
            $cards = $driver->findElements(WebDriverBy::cssSelector('div.relative.w-full.h-full.flex.flex-col'));
            $totalCards = count($cards);
            $originalWindow = $driver->getWindowHandle();
            for ($i = 0; $i < $totalCards; $i++) {
                $currentCards = $driver->findElements(WebDriverBy::cssSelector('div.relative.w-full.h-full.flex.flex-col'));
                ...
                fwrite($linkFile, $count . ': ' . $jobUrl . PHP_EOL);
                
                $count++;
            }

            try {
                $nextButton = $driver->findElement(WebDriverBy::cssSelector('li.next a, a.page-link[aria-label="Next"], .icon-arrow-right'));
